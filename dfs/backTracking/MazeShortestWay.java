...
public class MazeShortestWay {
	...
	public static void DfsMaze(int x, int y, int mazeLength){
		...
		if(x == terminalIndex && y == terminalIndex){
			wayCnt++;
			System.out.println(wayCnt + "번째 경로 길이: " + wayLength);
			if(wayCnt == 1){ //첫 번째 경로라면
				shortestWayLength = wayLength;
			}else{
				if(shortestWayLength > wayLength){
					shortestWayLength = wayLength;
				}
			}
			wayLength--;
		...
		maze[x][y] = 1; //다시 1로 되돌려 놓고 
		wayLength--; //길 하나 빼주고 리턴
		return;
	}
	...
	public static void main(String[] args) {
		...
		//3. 경로 수 출력
		System.out.println("최단경로 길이: " + shortestWayLength);
